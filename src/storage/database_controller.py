...
def save_library():
    ...
    for i, lib_item in enumerate(full_lib):
        if i > fav_index:
            songs = api_serv.get_album_songs_info(
                lib_item["id"], lib_item["size"], storing=True
            )
        elif i == fav_index:
            songs = api_serv.get_favourite_songs_info(storing=True)
        else:
            songs = api_serv.get_playlist_songs_info(
                lib_item["id"], lib_item["size"], storing=True
            )
        library.insert(
            {"name": lib_item["name"], "size": lib_item["size"], "songs": songs}
        )
    content = library.all()
