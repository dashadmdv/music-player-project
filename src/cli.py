...
class CLIDialogue:
    ...
    def run(self):
        ...
        while True:
            try:
                nav_choice = int(
                    input(
                        "Choose a folder to open(1 - search, 2 - my library, 3 - settings), "
                        + (
                            "9 - open player, "
            ...
            if nav_choice == 0:
                try:
                    exit_choice = int(
                        input("Are you sure you want to exit app? 0 - yes, 1 - no: ")
                    )
                    if exit_choice == 0:
                        print("Goodbye then!")
                        break
    ...
    def auth_dialogue(self, simplified=False):
        if not self.api_serv.refresh_token:
            while True:
                try:
                    if not simplified:
                        auth_choice = int(
                            input(
                                "Hello! You are not authorized :( Do you want to sign in? yes - 1, no - 0: "
                            )
                        )
                    else:
                        auth_choice = 1
            ...
            if auth_choice == 1:
                print(
                    "You will now be redirected to the browser. "
                ...
                print(
                    "If you are not authorized, you will not have access to your library then. You can sign in "
    ...
    def search_dialogue(self):
        ...
        try:
            search_type = int(
                input("What are you looking for? 1 - track, 2 - album, 3 - playlist: ")
            )
        except ValueError:
            search_type = 1
        if search_type == 1:
        ...
        if search_type == "track":
            ...
            while True:
                try:
                    choice = int(
                        input(f"Select {search_type}(by index), go back - 0: ")
                    )
                except ValueError:
                    continue
                if choice == 0:
                    ...
                    continue
                playlist = Playlist(
                    id=str(search_items[choice - 1]), album=(search_type == "album")
                )
    ...
    def library_dialogue(self):
        while True:
            try:
                prompt = (
                    "Choose what you want to open(whole library - 1, only my playlists - 2, local songs - 3), or"
                ...
                if not self.user:
                    print(
                        "You don't have full Spotify library access because you are not authorized :( "
                        + "If you want full access you can log in in the SETTINGS folder!"
                    )
                    prompt = (
                        "Choose what you want to do: open local songs - 3, "
                        + (
                            "9 - open player, "
                            if self.pl.check_if_was_playing()
                            else ""
                ...
                if check_if_library_exists() and not connect():
                    ...
                    prompt = (
                        "Choose what you want to open(whole library - 1, local songs - 3), "
                        + (
                            "9 - open player, "
                            if self.pl.check_if_was_playing()
                            else ""
            ...
            if show_choice == 0:
                ...
                if not self.user:
                    print(
                        "Authorize if you want to access your library :) You can do it in the SETTINGS folder!"
                    )
                    continue
                if self.user and not connect():
                    self.offline_lib_dialogue()
                print(self.api_serv.get_user_library_info())
                ...
                while True:
                    try:
                        public_choice = int(
                            input(
                                "Do you want playlist to be private - 0 or public - 1: "
                            )
                        )
            ...
            while True:
                ...
                if show_choice == 1:
                    ...
                    if library[choice - 1] != "favs":
                        playlist = Playlist(
                            id=str(library[choice - 1]),
                            album=((choice - 1) > library.index("favs")),
                        )
                else:
    ...
    def offline_lib_dialogue(self):
        ...
        while True:
            try:
                choice = int(
                    input("Select playlist(by index) to see info about, go back - 0: ")
                )
            except ValueError:
                continue
        ...
        while True:
            try:
                ...
                extended_interactions = (
                    "4 - change info, 5 - delete playlist, "
                    if (self.user and owned)
                    else ""
                )
                choice = int(
                    input(
            ...
            if choice == 0:
                ...
                if not playlist.is_unavailable():
                    thread = Thread(
                        target=self.pl.play,
                        args=(playlist.songs[0], playlist.songs),
                        daemon=True,
                    )
                    thread.start()
                    self.playback_dialogue()
                    ...
                    print(
                        "It seems like this playlist is unavailable for listening! Sorry :("
                    )
            elif choice == 4:
                new_name = None
                ...
                while True:
                    try:
                        name_choice = int(
                            input("Do you want to change name? 0 - yes, 1 - no: ")
                        )
                        if name_choice == 0:
                ...
                while True:
                    try:
                        description_choice = int(
                            input(
                                "Do you want to change description? 0 - yes, 1 - no: "
                            )
                        )
                ...
                while True:
                    try:
                        ...
                        public_choice = int(
                            input(
                                "Do you want to change playlist publicity? Current publicity: "
                                + ("public" if publicity else "private")
                                + ". 0 - yes, 1 - no: "
                            )
                        )
                        if public_choice == 0:
                ...
                self.api_serv.update_playlist_info(
                    playlist.id,
                    name=new_name,
                    public=new_public,
                ...
                while True:
                    try:
                        del_choice = int(
                            input(
                                "Are you sure you want to delete playlist? 0 - yes, 1 - no: "
                            )
                        )
                        if del_choice == 0:
                            ...
                            self.user.update_playlists()
                            print(
                                "You can restore deleted playlist for 90 days here: "
    ...
    def fav_songs_dialogue(self, playlist: list):
        while True:
            ...
            if choice == 0:
                ...
                thread = Thread(
                    target=self.pl.play, args=(playlist[0], playlist), daemon=True
                )
                thread.start()
                self.playback_dialogue()
            elif choice == 9 and self.pl.check_if_was_playing():
    ...
    def song_dialogue(self, song, playlist):
        ...
        while True:
            prompt = (
                ...
                + (
                    "3 - add to playlist, 4 - like, "
                    if type(song) != StorageSong and self.user
                    else ""
                )
                + "5 - add to queue, "
                + ("9 - open player, " if self.pl.check_if_was_playing() else "")
            ...
            if choice == 0:
                ...
                print(song.get_song_info())
            elif choice == 2:
                if playlist:
                    if type(playlist) == Playlist and not playlist.is_unavailable():
                        thread = Thread(
                            target=self.pl.play,
                            args=((song.id, song.source), playlist.songs),
                            daemon=True,
                        )
                        ...
                        if available:
                            thread = Thread(
                                target=self.pl.play,
                                args=((song.id, song.source), playlist),
                                daemon=True,
                            )
                    else:
                        print(
                ...
                while True:
                    try:
                        ...
                        pl_choice = int(
                            input(
                                "Select playlist to add song to(by index), go back - 0: "
                            )
                        )
                    except ValueError:
                        continue
                    self.api_serv.add_song_to_playlist(
                        self.user.playlists[pl_choice - 1], song.id
                    )
                    break
            elif choice == 4:
    ...
    def playback_dialogue(self):
        while True:
            ...
            if choice == 5:
                ...
                print(
                    "Repeat mode: "
                    + repeat_modes[self.pl.repeat_mode]
                    + f"({self.pl.repeat_mode})"
                )
                print(
                    "Shuffle mode: " + ("on" if self.pl.queue.shuffle_mode else "off")
                )
            if choice == 6:
                try:
                    mode = int(
    ...
    def settings_dialogue(self):
        while True:
            try:
                nav_choice = int(
                    input(
                        "Choose option(1 - log in, 2 - log out, 3 - choose folders to show local songs from), "
                        + (
                            "9 - open player, "
                            if self.pl.check_if_was_playing()
                            else ""
            ...
            if nav_choice == 0:
                ...
                if self.user:
                    print(
                        "You are authorized! If you want to log in as a different user you should LOG OUT first"
                    )
                    continue
                else:
                ...
                if not self.user:
                    ...
                    try:
                        exit_choice = int(
                            input("Are you sure you want to log out? 0 - yes, 1 - no: ")
                        )
                        if exit_choice == 0:
                ...
                try:
                    path_choice = int(
                        input(
                            "Do you want to add - 1 or delete - 2 source directories? go back - 0: "
                        )
                    )
                    if path_choice == 0:
                        continue
                        ...
                        while True:
                            try:
                                path_del_choice = int(
                                    input(
                                        "Select directory(by index) to delete, go back - 0: "
                                    )
                                )
                            except ValueError:
                                continue
